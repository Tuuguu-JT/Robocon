...
void moveCalc(){
  ...
  spdA=(Z*sin((225-angle)*PI/180)+r)*maxspeed/255;
  spdB=(Z*sin((135-angle)*PI/180)+r)*maxspeed/255;
  if(spdA > 255){spdA=255;}
  if(spdA < -255){spdA=-255;}
  if(spdB > 255){spdB=255;}
  if(spdB < -255){spdB=-255;}
  if(spdA<range && spdA>-range){spdA=0;}
  if(spdB<range && spdB>-range){spdB=0;}
}
/*--------------------------PID Calculation-------------------------*/
void pidCalc(){
